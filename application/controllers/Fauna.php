...
class Fauna extends CI_Controller {
    ...
    public function read() {
        ...
        foreach ($rows as $i) {
            ...
            $population = 0;
            $getPopulation = $this->m_data->runQuery('SELECT SUM(population) as population FROM fauna_population WHERE fauna_uuid = "'.$i->uuid.'"')->result();
    ...
    public function increasePopulation() {
        try {
            ...
            $this->m_data->save($data, 'fauna_population');
    ...
    public function readPopulation() {
        ...
        $rows = $this->m_data->getRows(
            $_POST,
            array(null, 'uuid', 'fauna_uuid', 'year', 'population'),
            array('year', 'population'),
            array('year' => 'ASC'),
            'fauna_population',
            'uuid, fauna_uuid, year, population',
            NULL,
            array(
                array(
                    'parameters' => 'and',
        ...
        $result = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->m_data->countAll('SELECT uuid, fauna_uuid, year, population
                FROM fauna_population
                WHERE fauna_uuid = "'.$fauna_uuid.'"
                ORDER BY year ASC'),
            "recordsFiltered" => $this->m_data->countFiltered(
                $_POST,
                array(null, 'uuid', 'fauna_uuid', 'year', 'population'),
